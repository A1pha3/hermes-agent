...
> **📚 学习目标**
> 阅读本文后，你将能够：
> - 理解Hermes Agent的版本号规则和发布周期规划
> - 掌握正式版本的完整发布流程和检查清单
> - 掌握补丁版本的紧急发布流程和回滚机制
> - 确保发布的版本质量稳定，符合生产环境要求

本文档介绍 Hermes Agent 的版本发布流程，仅核心维护者可以执行发布操作，贡献者可以了解版本发布标准。
...
## 回滚流程
如果发布后发现严重问题，需要紧急回滚：
1. 从 PyPI 删除有问题的版本，防止更多用户升级
2. 发布公告通知用户不要升级，或降级到上一个稳定版本
3. 定位问题根源，修复后发布补丁版本
4. 发布详细公告说明问题、影响范围和解决方案

---

## ✍️ 练习
1. 模拟一个次版本发布流程，从创建发布分支到发布GitHub Release的全流程操作
2. 编写一个补丁版本的发布说明，模拟修复一个安全漏洞的场景
3. 设计一个发布前检查自动化脚本，自动运行测试、检查版本号、构建产物等
4. 制定一个发布后的监控方案，确保发布后及时发现问题

---

## ❓ 常见问题
### Q：预发布版本和正式版本的区别是什么？
A：预发布版本用于内部和公开测试，不建议生产环境使用：
- `alpha`：功能不完整，仅内部测试
- `beta`：功能完整，公开测试，可能存在bug
- `rc`：功能冻结，仅修复blocker级别bug，接近正式版本
正式版本经过完整测试，可以用于生产环境。

### Q：发布后发现小bug要不要立即发布补丁版本？
A：根据bug的严重程度判断：
- 严重bug（安全漏洞、核心功能不可用、影响大量用户）：立即发布补丁版本
- 普通bug（不影响核心功能、有 workaround）：收集多个bug后统一发布补丁版本，避免频繁发布打扰用户

### Q：版本发布后还能修改吗？
A：已经上传到PyPI的版本不能修改，只能发布新的版本：
- PyPI不允许覆盖已经上传的版本，即使删除后也不能重新上传相同版本号的包
- 如果发现发布错误，只能升级版本号重新发布
- GitHub Release可以编辑内容，但是构建产物不能修改

### Q：怎么处理跨大版本的升级？
A：大版本升级会有不兼容变更，需要提供：
1. 详细的升级指南，列出所有不兼容变更和迁移步骤
2. 自动迁移工具，帮助用户自动升级配置和数据
3. 过渡期支持，保留旧API的兼容层至少一个次版本周期
4. 提前发布多个预发布版本，让用户提前测试

### Q：发布版本时需要冻结代码多久？
A：根据版本类型不同：
- 主版本/次版本：功能冻结至少1周，只修复blocker级别bug
- RC版本：冻结至少3天，无严重问题即可发布正式版
- 补丁版本：冻结24小时，验证修复正确且无regression即可发布

---

## 📖 导航
- 上一篇：[测试指南](./6-testing.md)
- 下一篇：[架构概述](../architecture/1-overview.md)
- [返回目录](../SUMMARY.md)
