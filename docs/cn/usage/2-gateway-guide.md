...
**为什么需要消息网关？**
不需要为每个消息平台单独开发和部署AI助手，同一套Agent逻辑可以同时在多个平台提供服务，大大降低开发和维护成本，同时统一了所有平台的AI能力和体验。
...
## ✍️ 练习
1. 选择一个你常用的消息平台（比如 Telegram、飞书），完成网关配置并启动
2. 配置用户白名单，测试非授权用户无法访问
3. 将网关安装为系统服务，测试开机自动启动功能
4. 配置后台任务通知级别为 `error`，仅在任务出错时收到通知
...
### Q：网关的日志存在哪里？
A：前台运行时日志直接输出到终端，安装为系统服务后日志位于：
- Linux：`/var/log/hermes-gateway.log`
- macOS：`~/Library/Logs/hermes-gateway.log`

---

## 📖 导航
- 上一篇：[CLI 使用指南](./1-cli-guide.md)
- 下一篇：[工具使用指南](./3-tools-guide.md)
- [返回目录](../SUMMARY.md)
